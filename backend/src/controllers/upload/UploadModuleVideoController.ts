...
export class UploadCourseModuleController extends BaseController {
  ...
  protected async executeImpl(req: Request, res: Response) {
    try {
      ...
      if (totalVideo > 3)
        return this.forbidden(res, "You cannot upload more than 3 video");
      let fileName;
      files.forEach((file: any) => {
        ...
        file.mv(
          `${path.dirname(require.main?.filename!)}/upload/course/${fileName}`,
          (err: any) => {
            if (err) return this.fail(res, err.toString());
          }
        );
