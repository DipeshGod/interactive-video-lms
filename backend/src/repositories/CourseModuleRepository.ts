...
export class CourseModuleRepository implements ICourseModuleRepository {
  ...
  public getCourseModuleByCourseId(
    id: string
  ): Query<ICourseModuleDoc[], ICourseModuleDoc, {}> {
    try {
  ...
  public editCourseModule(
    id: string,
    moduleData: ICourseModuleDoc | any
  ): Query<ICourseModuleDoc | null, ICourseModuleDoc, {}> {
    try {
  ...
  public getCourseModuleByModuleId(
    id: string
  ): Query<ICourseModuleDoc | null, ICourseModuleDoc, {}> {
    try {
  ...
  public getCurrentModuleByCourseId(courseId: string) {
    try {
      const courseModule = this.model.find({ courseId: courseId }).sort(-1);
      return courseModule;
    } catch (err: any) {
      throw new Error(err.toString());
    }
