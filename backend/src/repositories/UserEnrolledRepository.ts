import { IUserEnrolled, IUserEnrolledModel } from "../interfaces/models/UserEnrolled";
...
export class UserEnrolledRepository implements IUserEnrolledRepository {
    ...
    public getUserEnrolled(id: string): any {
        try {
            const userEnrolled = this.model.find({ userId: id })
            return userEnrolled;
        } catch (err: any) {
            throw new Error(err.toString());
        }
    ...
    public editUserEnrolled(id: string, userEnrolledData: IUserEnrolled): any {
        try {
            const userEnrolled = this.model.findOneAndUpdate({ userId: id }, userEnrolledData, { new: true });
            return userEnrolled;
        } catch (err: any) {
            throw new Error(err.toString());
        }
